...
#ifndef CPUTILITIES_SWIFT_H
#define CPUTILITIES_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="CPUtilities",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
SWIFT_CLASS("_TtC11CPUtilities13CPServiceInfo")
@interface CPServiceInfo : NSObject
/// Helper function to make creation of ServiceInfo a bit easier.
/// \param type Type of the service.
///
/// \param push Should the service subscribe to mobile push notifications.
///
+ (CPServiceInfo * _Nonnull)buildWithType:(enum ServiceType)type push:(BOOL)push SWIFT_WARN_UNUSED_RESULT;
...
/// All services must comply with this protocol. This protocol provides basic functionality to be used by the CPaaS service provider.
SWIFT_PROTOCOL("_TtP11CPUtilities17CPServiceProtocol_")
...
typedef SWIFT_ENUM(NSInteger, CallEventType, open) {
  CallEventTypeRinging = 0,
  CallEventTypeSessionEnded = 1,
  CallEventTypeCancelled = 2,
  CallEventTypeTransferCompleted = 3,
  CallEventTypeTransferFailed = 4,
};

typedef SWIFT_ENUM(NSInteger, CallNotificationType, open) {
  CallNotificationTypeEvent = 0,
  CallNotificationTypeAnswer = 1,
...
typedef SWIFT_ENUM(NSInteger, ServiceType, open) {
  ...
  ServiceTypeChat = 1,
  ServiceTypeCall = 2,
  ServiceTypePresence = 3,
  ServiceTypeAddressbook = 4,
};
